...
def fetch_city_aq(city, start_date, end_date):
    ...
    try:
        ...
        logger.info(f"  ✓ {len(df_daily)} days")
...
def generate_synthetic_data():
    ...
    for city in CITIES:
        ...
        all_data.append(df)
        logger.info(f"  ✓ {city}: {len(df)} days (synthetic)")
...
def save_data(df, synthetic=False):
    ...
    try:
        ...
        if existing == 0:
            insert_city_data(engine, df)
            logger.info(f"✓ Inserted {len(df)} rows")
        else:
            logger.warning(f"{existing} rows exist, skipping")
    except Exception as e:
        logger.error(f"DB: {e}")
...
def main():
    ...
    if not all_data:
        logger.warning("No data. OpenAQ may require authentication or have gaps.")
